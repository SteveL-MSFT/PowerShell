...
namespace Microsoft.PowerShell
{
    ...
    internal class CommandLineParameterParser
    {
        ...
        internal bool ConfigurationGet
        {
            get
            {
                AssertArgumentsParsed();
        ...
        internal string? ConfigurationSet
        {
            get
            {
                AssertArgumentsParsed();
        ...
        private void ParseHelper(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; ++i)
            {
                ...
                else if (MatchSwitch(switchKey, "configurationget", "configurationget"))
                {
                    ...
                    _configurationGet = true;
                    ParametersUsed |= ParameterBitmap.ConfigurationGet;
                }
                else if (MatchSwitch(switchKey, "configurationset", "configurationset"))
                {
                    ++i;
                    if (i >= args.Length)
                    {
                        SetCommandLineError(
                            CommandLineParameterParserStrings.MissingConfigurationSetArgument);
                        break;
                    }

                    _configurationSet = args[i];
                    ParametersUsed |= ParameterBitmap.ConfigurationSet;
                }
